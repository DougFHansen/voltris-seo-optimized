...
export function calculateInternalLinks(
  ...
  allGuides.forEach(guide => {
    ...
    // 3. PALAVRAS-CHAVE COMPLEMENTARES (peso: 2.0) - INTENÇÃO COMPLEMENTAR
    const currentKeywords = (currentGuide as any).keywords || [];
    const guideKeywords = (guide as any).keywords || [];
    const keywordOverlap = currentKeywords.some((k: string) => 
      guideKeywords.some((gk: string) => k.includes(gk.split(' ')[0])) &&
