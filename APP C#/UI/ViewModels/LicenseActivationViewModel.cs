...
namespace VoltrisOptimizer.UI.ViewModels
{
    ...
    public class LicenseActivationViewModel : INotifyPropertyChanged
    {
        ...
        public string LicenseKey
        {
            ...
            set
            {
                if (_licenseKey != value)
                {
                    ...
                    OnPropertyChanged(nameof(IsActivateButtonVisible));
                    ShowError = false;
                }
            }
        }
        ...
        private ObservableCollection<LicensePlanViewModel>? _licensePlans;
        public ObservableCollection<LicensePlanViewModel> LicensePlans
        {
            get
            {
                if (_licensePlans == null)
                {
                    var dispatcher = System.Windows.Application.Current?.Dispatcher;
                    if (dispatcher != null && !dispatcher.CheckAccess())
                    {
                        dispatcher.Invoke(() => _licensePlans = new ObservableCollection<LicensePlanViewModel>());
                    }
                    else
                    {
                        _licensePlans = new ObservableCollection<LicensePlanViewModel>();
                    }
                }
                return _licensePlans;
            }
        }
        ...
        public string ActiveLicenseTitle
        {
            get
            {
                if (CurrentState == null || CurrentState.Type == LicenseType.Trial) return "Licença";
                return $"Licença {CurrentState.Type} Ativa";
            }
        }
        ...
        public bool IsActivateButtonVisible
        {
            get
            {
                 if (string.IsNullOrEmpty(CurrentState?.LicenseKey)) return true;
                
                 return LicenseKey != CurrentState.LicenseKey;
            }
        }
        ...
        public async Task InitializeAsync()
        {
            try
            {
                ...
                // Sempre verificar a chave existente, independente do cache
                var existingKey = VoltrisOptimizer.Services.LicenseManager.Instance.GetCurrentLicenseKey();
                if (!string.IsNullOrEmpty(existingKey) && !existingKey.ToUpper().StartsWith("TRIAL"))
                {
                    LicenseKey = existingKey;
                    CurrentState.LicenseKey = existingKey;
                    
                    // Se temos chave mas o estado não estava ativado (ex: cache inválido), ativar na UI
                    if (!CurrentState.IsActivated)
                    {
                        // Validar se é realmente válida
                        if (await VoltrisOptimizer.Services.LicenseManager.Instance.IsLicenseValidAsync())
                        {
                            // CRÍTICO: Re-ativar via Manager para atualizar IsPro e disparar eventos para o Dashboard
                            var storedKey = VoltrisOptimizer.Services.LicenseManager.Instance.GetCurrentLicenseKey();
                            if (!string.IsNullOrEmpty(storedKey))
                            {
                                await VoltrisOptimizer.Services.LicenseManager.Instance.ActivateLicenseAsync(storedKey);
                            }

                            var info = VoltrisOptimizer.Services.LicenseManager.Instance.GetLicenseInfo();
                            if (info != null)
                            {
                                CurrentState.IsActivated = true;
                                CurrentState.Type = LicensePlan.ParseType(info.PlanType);
                                CurrentState.ExpiresAt = info.ExpiryDate;
                                CurrentState.MaxDevices = info.MaxDevices;
                                
                                // Importante: Salvar cache para corrigir outros componentes (Dashboard)
                                _ = Task.Run(() => {
                                    LicenseCacheService.Instance.SaveLicenseCache(new LicenseCacheData
                                    {
                                        LicenseKey = info.LicenseKey,
                                        LicenseType = info.PlanType,
                                        IsValid = true,
                                        MaxDevices = info.MaxDevices,
                                        DevicesInUse = info.ActiveDevices,
                ...
                OnPropertyChanged(nameof(ActiveLicenseTitle));
                OnPropertyChanged(nameof(IsActivateButtonVisible));
        ...
        public async Task ActivateLicenseAsync()
        {
            ...
            try
            {
                ...
                // Priorizar validação LOCAL (Offline) para suportar LicenseGenerator
                // Se a chave tiver uma assinatura válida, ativamos imediatamente sem consultar API
                var localResult = await VoltrisOptimizer.Services.LicenseManager.Instance.ActivateLicenseAsync(licenseKey);
                
                if (localResult.Success)
                {
                    _logger.LogInfo($"[License] Ativação local bem-sucedida! Plano: {localResult.Plan}");
                    
                    // Converter string do plano para Enum
                    var licenseType = LicensePlan.ParseType(localResult.Plan);
                    
                    // Atualizar estado UI
                    CurrentState.IsActivated = true;
                    CurrentState.Type = licenseType;
                    CurrentState.MaxDevices = localResult.MaxDevices;
                    CurrentState.DevicesInUse = 1; // Ativação offline, assumimos 1
                    CurrentState.LicenseKey = licenseKey;
                    CurrentState.IsOfflineMode = true; // Ativado offline
                    
                    // Tentar extrair data da chave para exibir corretamente
                    try 
                    {
                        var parts = licenseKey.Split('-');
                        if (parts.Length > 3 && DateTime.TryParseExact(parts[3], "yyyyMMdd", null, System.Globalization.DateTimeStyles.None, out var expiration))
                        {
                            CurrentState.ExpiresAt = expiration;
                        }
                        else
                        {
                    ...
                    // Propriedade estática global já foi atualizada pelo LicenseManager
                    
                    ShowSuccess = true;
                    // Salvar cache para persistência e Dashboard
                    _ = Task.Run(() => {
                        LicenseCacheService.Instance.SaveLicenseCache(new LicenseCacheData
                        {
                            LicenseKey = licenseKey,
                            LicenseType = localResult.Plan,
                            IsValid = true,
                            MaxDevices = localResult.MaxDevices,
                            DevicesInUse = 1,
                            ExpiresAt = CurrentState.ExpiresAt
                        });
                    });

                    StatusMessage = $"✅ Licença {localResult.Plan.ToUpper()} ativada com sucesso!";
                    
                    _logger.LogSuccess($"[License] Licença ativada localmente com sucesso");
                    
                    await Task.Delay(1500);
                    App.TelemetryService?.TrackEvent("LICENSE_ACTIVATE", "License", localResult.Plan, success: true);
                    OnPropertyChanged(nameof(ActiveLicenseTitle));
                    OnPropertyChanged(nameof(IsActivateButtonVisible));
                ...
                // Se a ativação local falhar (ex: não é uma chave offline assinada, ou assinatura inválida),
                // Logar o motivo e tentar API como fallback (para chaves online puras)
                _logger.LogInfo($"[License] Ativação local não concluiu ({localResult.Message}). Tentando API...");
                ...
                if (response.Success)
                {
                    ...
                    await Task.Delay(1500);
                    App.TelemetryService?.TrackEvent("LICENSE_ACTIVATE", "License", response.Type, success: true);
                    OnPropertyChanged(nameof(ActiveLicenseTitle));
                    OnPropertyChanged(nameof(IsActivateButtonVisible));
                    ActivationSucceeded?.Invoke(this, EventArgs.Empty);
                }
                else
                {
                    ...
                    App.TelemetryService?.TrackEvent("LICENSE_ACTIVATE_FAIL", "License", errorCode, success: false);
        ...
        private void BuyLicense(object? parameter)
        {
            try
            {
                ...
                _logger.LogInfo($"[License] Abrindo página de aquisição de licença (Origem: {plan})");
                
                // URL única solicitada: https://voltris.com.br/adquirir-licenca
                // Todos os botões devem redirecionar para cá
                var licenseUrl = "https://voltris.com.br/adquirir-licenca";
                ...
                _logger.LogInfo($"[License] Redirecionamento realizado: {licenseUrl}");
                StatusMessage = "Abrindo site oficial...";
            }
            catch (Exception ex)
            {
