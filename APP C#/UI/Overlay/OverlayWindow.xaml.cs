...
namespace VoltrisOptimizer.UI.Overlay
{
    ...
    public partial class OverlayWindow : Window
    {
        ...
        {
            InitializeComponent();
            RoundedWindowHelper.Apply(this, 5);
            _settings = settings;
        ...
        public void UpdateMetrics(MetricsData metrics)
        {
            Dispatcher.Invoke(() =>
            {
                lock (_lock)
                {
                    ...
                    // FPS — sempre mostrar quando ativado, mesmo se 0 (aguardando dados ETW)
                    if (_settings.Metrics.ShowFps)
                    {
                        var fpsText = metrics.Fps > 0 ? $"FPS: {metrics.Fps:F1}" : "FPS: ---";
                        UpdateOrCreateMetric("FPS", fpsText);
                    }
                    else
                    {
                        RemoveMetric("FPS");
                    ...
                    // FrameTime — sempre mostrar quando ativado
                    if (_settings.Metrics.ShowFrameTime)
                    {
                        var ftText = metrics.FrameTimeMs > 0 ? $"FrameTime: {metrics.FrameTimeMs:F2} ms" : "FrameTime: --- ms";
                        UpdateOrCreateMetric("FrameTime", ftText);
                    }
                    else
                    {
                    ...
                    // CORREÇÃO: CPU Temp — mostrar "---" quando valor ainda não disponível
                    // NÃO remover a métrica do overlay quando o toggle está ativo
                    if (_settings.Metrics.ShowCpuTemperature)
                    {
                        var cpuTempText = metrics.CpuTemperature.HasValue
                            ? $"CPU Temp: {metrics.CpuTemperature.Value:F1}°C"
                            : "CPU Temp: ---°C";
                        UpdateOrCreateMetric("CPUTemp", cpuTempText);
                    }
                    else
                    {
                    ...
                    // CORREÇÃO: GPU Temp — mostrar "---" quando valor ainda não disponível
                    if (_settings.Metrics.ShowGpuTemperature)
                    {
                        var gpuTempText = metrics.GpuTemperature.HasValue
                            ? $"GPU Temp: {metrics.GpuTemperature.Value:F1}°C"
                            : "GPU Temp: ---°C";
                        UpdateOrCreateMetric("GPUTemp", gpuTempText);
                    }
                    else
                    {
                    ...
                    // CORREÇÃO: CPU Clock — mostrar "---" quando valor ainda não disponível
                    if (_settings.Metrics.ShowCpuClock)
                    {
                        var cpuClockText = metrics.CpuClockMhz.HasValue
                            ? $"CPU Clock: {metrics.CpuClockMhz.Value:F0} MHz"
                            : "CPU Clock: --- MHz";
                        UpdateOrCreateMetric("CPUClock", cpuClockText);
                    }
                    else
                    {
                    ...
                    // CORREÇÃO: GPU Clock — mostrar "---" quando valor ainda não disponível
                    if (_settings.Metrics.ShowGpuClock)
                    {
                        ...
                        {
                            UpdateOrCreateMetric("GPUClock", "GPU Core: --- MHz");
                        }
                    }
                    else
                    {
                    ...
                    // CORREÇÃO: Input Latency — mostrar "---" quando valor ainda não disponível
                    if (_settings.Metrics.ShowInputLatency)
                    {
                        var latencyText = metrics.InputLatencyMs.HasValue
                            ? $"Input Latency: {metrics.InputLatencyMs.Value:F2} ms"
                            : "Input Latency: --- ms";
                        UpdateOrCreateMetric("InputLatency", latencyText);
                    }
                    else
                    {
