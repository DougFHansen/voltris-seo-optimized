...
namespace VoltrisOptimizer.Core.SystemIntelligenceProfiler
{
    public class RollbackManager : IRollbackManager
    {
        ...
        {
            App.LoggingService?.LogWarning($"[ROLLBACK] Executando rollback para o ponto: {pointId}");
            return await Task.Run(() =>
            {
                ...
                App.LoggingService?.LogSuccess($"[ROLLBACK] O processo de rollback foi concluído para {pointId}");
                return result;
            });
        }
