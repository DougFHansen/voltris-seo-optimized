...
namespace VoltrisOptimizer.Services.Gamer.GamerModeManager
{
    ...
    public class GamerModeManager : IGamerModeManager
    {
        ...
        private readonly CpuProfileOptimizationService? _cpuProfile;
        ...
        public GamerModeManager(ILoggingService logger, IGameDetectionService gameDetectionService,
            ITimerResolutionService? timerResolution = null,
            CpuProfileOptimizationService? cpuProfile = null)
        {
            ...
            _cpuProfile = cpuProfile ?? Core.ServiceLocator.GetService<CpuProfileOptimizationService>();
        ...
        public async Task<bool> ActivateAsync(string? gameExecutable = null, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // 1.6. Aplicar configurações de CPU via powrprof.dll para o perfil Gamer.
                // Isso define: min/max processor state, core parking, boost mode, EPP.
                // Deve ser chamado DEPOIS do plano de energia estar ativo.
                if (_cpuProfile != null)
                {
                    Log(LogLevel.Info, "Aplicando configurações de CPU (powrprof)...", "CpuProfile");
                    try
                    {
                        await _cpuProfile.ApplyForCurrentProfileAsync();
                        Log(LogLevel.Info, "✓ Configurações de CPU aplicadas", "CpuProfile");
                    }
                    catch (Exception ex)
                    {
                        Log(LogLevel.Warning, $"CpuProfile falhou (não crítico): {ex.Message}", "CpuProfile");
                    }
                }
        ...
        // MMCSS (Multimedia Class Scheduler Service) — API nativa
        // Registra o processo na classe "Games" para garantir prioridade de scheduler.
        // Documentado pela Microsoft: https://learn.microsoft.com/en-us/windows/win32/procthread/multimedia-class-scheduler-service
        ...
        private void ActivateTimerAndMmcss()
        {
            ...
            // Isso instrui o scheduler do Windows a tratar este processo com prioridade
            // equivalente a aplicações multimídia, reduzindo preempção indesejada.
            try
            {
                _mmcssTaskIndex = 0;
                _mmcssHandle = AvSetMmThreadCharacteristics("Games", ref _mmcssTaskIndex);
                if (_mmcssHandle != IntPtr.Zero)
                    Log(LogLevel.Info, $"✓ MMCSS registrado na classe 'Games' (TaskIndex={_mmcssTaskIndex})", "MMCSS");
                else
                    Log(LogLevel.Warning, $"MMCSS falhou (avrt.dll) — Win32Error={Marshal.GetLastWin32Error()}", "MMCSS");
            }
            catch (Exception ex)
            {
                Log(LogLevel.Warning, $"MMCSS não disponível: {ex.Message}", "MMCSS");
            }
        }
        ...
        private void DeactivateTimerAndMmcss()
        {
            ...
            if (_mmcssHandle != IntPtr.Zero)
            {
                try
                {
                    ...
                    Log(LogLevel.Info, "MMCSS desregistrado", "MMCSS");
                }
                catch { }
            }
        }
