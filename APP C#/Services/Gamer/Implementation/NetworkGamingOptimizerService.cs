    ...
    public class NetworkGamingOptimizerService : INetworkGamingOptimizer
    {
        ...
        private void ApplyTcpTweaks()
        {
            try
            {
                ...
                // TCP ACK Frequency = 2 (Recomendação Enterprise: Balanceado Latência/Estabilidade)
                // Valor 1 é muito agressivo para Upload (Streamers)
                key.SetValue("TcpAckFrequency", 
                    2, 
                    RegistryValueKind.DWord);
                ...
                _logger.LogInfo("[NetworkOptimizer] TCP tweaks aplicados (Ack=2, NoDelay=1)");
            }
            catch (Exception ex)
            {
