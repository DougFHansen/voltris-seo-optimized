...
namespace VoltrisOptimizer.Services.Gamer
{
    ...
    public class IntelligentBackgroundSuspender : IDisposable
    {
        ...
        // Debounce para trocas de contexto — evita suspend/resume cycling
        private DateTime _lastSuspendTime = DateTime.MinValue;
        private DateTime _lastResumeTime = DateTime.MinValue;
        private const int SUSPEND_RESUME_COOLDOWN_MS = 5000; // 5 segundos entre operações
        ...
        /// <summary>
        /// Handler para detecção de troca de contexto — com debounce para evitar cycling
        /// </summary>
        private void OnAppSwitchDetected(object? sender, AppSwitchEventArgs e)
        {
            try
            {
                ...
                {
                    // Debounce: não suspender se acabamos de retomar processos
                    var sinceLastResume = (now - _lastResumeTime).TotalMilliseconds;
                    if (sinceLastResume < SUSPEND_RESUME_COOLDOWN_MS)
                    {
                        _logger.LogInfo($"[BgSuspender] Suspensão ignorada (cooldown {sinceLastResume:F0}ms desde último resume)");
                        return;
                    }
                ...
                {
                    // Debounce: não retomar se acabamos de suspender processos
                    var sinceLastSuspend = (now - _lastSuspendTime).TotalMilliseconds;
                    if (sinceLastSuspend < SUSPEND_RESUME_COOLDOWN_MS)
                    {
                        _logger.LogInfo($"[BgSuspender] Resume ignorado (cooldown {sinceLastSuspend:F0}ms desde última suspensão)");
                        return;
                    }
        ...
        private bool ShouldSuspendProcess(Process process)
        {
            try
            {
                // CORREÇÃO: Não re-suspender processos já suspensos (evita syscalls redundantes)
                if (_suspendedProcesses.ContainsKey(process.Id))
                    return false;
        ...
        private bool ResumeProcess(int processId)
        {
            try
            {
                ...
                {
                    // Verificar se o processo ainda existe antes de tentar retomar
                    try
                    {
                        ...
                        if (process.HasExited)
                        {
                            ...
                            _logger.LogInfo($"[BgSuspender] Processo já encerrado, removido da lista: {info.ProcessName} (PID: {processId})");
                            return true;
                        }
                    ...
                    {
                        // Processo não existe mais (PID inválido)
                        _suspendedProcesses.Remove(processId);
                        _logger.LogInfo($"[BgSuspender] Processo não encontrado (já encerrado): {info.ProcessName} (PID: {processId})");
                        return true;
                    }
                    catch (System.ComponentModel.Win32Exception)
                    {
                        // Acesso negado - processo protegido do sistema
                        _suspendedProcesses.Remove(processId);
                        _logger.LogInfo($"[BgSuspender] Acesso negado ao retomar (processo protegido): {info.ProcessName} (PID: {processId})");
                        return false;
                    }
            ...
            {
                // Remover da lista mesmo em caso de erro para evitar tentativas infinitas
                _suspendedProcesses.Remove(processId);
