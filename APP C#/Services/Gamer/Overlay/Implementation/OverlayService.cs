...
namespace VoltrisOptimizer.Services.Gamer.Overlay.Implementation
{
    ...
    public class OverlayService : IOverlayService, IDisposable
    {
        ...
        public OverlayService(ILoggingService? logger = null, IMetricsCollector? metricsCollector = null, IThermalMonitorService? thermalMonitor = null)
        {
            ...
            // CORREÇÃO: Carregar configurações na inicialização
            try
            {
                _settings = OverlaySettings.LoadFromFile(_settingsPath);
                _logger?.LogInfo($"[Overlay] Configurações carregadas. IsEnabled={_settings.IsEnabled}");
            }
            catch (Exception ex)
            {
                _logger?.LogWarning($"[Overlay] Erro ao carregar configurações, usando padrões: {ex.Message}");
        ...
        public async Task<bool> StartAsync(int gameProcessId, CancellationToken cancellationToken = default)
        {
            try
            {
                _logger?.LogInfo($"[Overlay] StartAsync chamado para processo {gameProcessId}");
                
                lock (_lock)
                {
                    _logger?.LogInfo($"[Overlay] Estado atual: _isActive={_isActive}, _currentGameProcessId={_currentGameProcessId}");
                    
                    if (_isActive && _currentGameProcessId == gameProcessId)
                    {
                        _logger?.LogInfo("[Overlay] Overlay já está ativo para este processo");
                        return true; // Já está ativo para este processo
                    }

                    if (_isActive)
                    {
                        _logger?.LogInfo("[Overlay] Parando overlay anterior");
                        // Parar overlay anterior se houver
                        StopAsync().Wait(1000);
                    }

                    _logger?.LogInfo($"[Overlay] Verificando se está habilitado: IsEnabled={_settings.IsEnabled}");
                    
                    if (!_settings.IsEnabled)
                    {
                        _logger?.LogWarning("[Overlay] Overlay desabilitado nas configurações");
                        return false;
                    }
                ...
                _logger?.LogInfo($"[Overlay] Verificando se processo {gameProcessId} existe");
                try
                {
                    ...
                    _logger?.LogInfo($"[Overlay] Processo {gameProcessId} ({process.ProcessName}) encontrado e ativo");
                }
                catch (Exception ex)
                {
                    _logger?.LogWarning($"[Overlay] Processo {gameProcessId} não encontrado: {ex.Message}");
                    return false;
                }

                // Iniciar coletor de métricas
                _logger?.LogInfo($"[Overlay] Iniciando coletor de métricas para processo {gameProcessId}");
                await _metricsCollector.StartAsync(gameProcessId, cancellationToken);

                // Criar e mostrar janela de overlay
                _logger?.LogInfo("[Overlay] Criando janela de overlay");
                await Application.Current.Dispatcher.InvokeAsync(() =>
                {
                    _overlayWindow = new OverlayWindow(_settings);
                    _overlayWindow.Show();
                    _logger?.LogInfo("[Overlay] Janela de overlay criada e exibida");
                });
                ...
                _logger?.LogSuccess($"[Overlay] Overlay iniciado com sucesso para processo {gameProcessId}");
            ...
            catch (Exception ex)
            {
                ...
                _logger?.LogError($"[Overlay] StackTrace: {ex.StackTrace}");
                return false;
            }
        }
        ...
        public void Dispose()
        {
            ...
            _ = Task.Run(async () => 
            {
                ...
                {
                    // Log silently - Dispose não deve lançar exceções
                    System.Diagnostics.Debug.WriteLine($"[Overlay] Erro no Dispose: {ex.Message}");
                }
            }, cts.Token);
