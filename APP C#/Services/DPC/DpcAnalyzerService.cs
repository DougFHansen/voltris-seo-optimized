...
namespace VoltrisOptimizer.Services.DPC
{
    ...
    public class DpcAnalyzerService : IDpcAnalyzerService, IDisposable
    {
        ...
        public void Start()
        {
            // OTIMIZAÇÃO DE PERFORMANCE: Intervalo aumentado de 500ms para 1000ms
            // Reduz overhead de CPU sem impactar qualidade da análise DPC
            _timer = new Timer(Collect, null, 0, 1000);
            _logger.LogInfo("[DPC] Monitor iniciado (intervalo: 1s)");
        }
        public void Stop()
        {
            _timer?.Dispose();
