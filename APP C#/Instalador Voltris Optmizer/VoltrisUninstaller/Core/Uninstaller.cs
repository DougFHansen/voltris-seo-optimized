    ...
    public class Uninstaller
    {
        ...
        private async Task RemoveInstallationFoldersAsync(string? detectedInstallPath, IProgress<UninstallProgress>? progress = null)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    // Lista de caminhos possÃ­veis em Program Files
                    ...
                    })
                    {
                        ...
                        try
                        {
                            ...
                            // Buscar diretamente por pastas que comeÃ§am com "Voltris"
                    ...
                    // Nota: Se chegamos aqui, o desinstalador jÃ¡ foi movido para temp (se necessário) no Program.cs
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    var folderNames = new[]
                    {
                        ...
                        "Voltris Corporation",
                        "Voltris Optimizer",
                        "VoltrisOptimizer", // Sem espaÃ§o
                        "VoltrisOptmizer", // Com typo (sem 'i')
                        "Voltris Optmizer", // Com typo e espaÃ§o
                        CompanyName,
                        ProductName
                    };
                    ...
                    foreach (var basePath in basePaths)
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var dir in allDirs)
                            {
                                ...
                                // Verificar se contÃ©m "Voltris" ou variações
                                if (dirName.Contains("Voltris", StringComparison.OrdinalIgnoreCase) ||
                                    (dirName.Contains("Optimizer", StringComparison.OrdinalIgnoreCase) && 
                                     dirName.Contains("Opt", StringComparison.OrdinalIgnoreCase)) ||
                                    dirName.Contains("Optmizer", StringComparison.OrdinalIgnoreCase))
                                {
                                    // Verificar se jÃ¡ não foi removida
        ...
        private void RemoveDirectoryRecursive(string directory, int maxRetries = 5)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                ...
                // Esperar antes de tentar novamente (reduzido para ser mais rÃ¡pido)
                System.Threading.Thread.Sleep(500 * attempt);
            }
        ...
        private void RemoveDirectoryRecursiveFast(string directory, int maxRetries = 3)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    // Tentar remover diretamente primeiro (mais rÃ¡pido)
                    try
                    {
                        Directory.Delete(directory, true);
        ...
        private void RemoveDirectoryContentsExceptSelf(string directory, string excludeFile)
        {
            ...
            try
            {
                ...
                // Remover todos os diretórios exceto o que contÃ©m o desinstalador
                var dirs = Directory.GetDirectories(directory, "*", SearchOption.AllDirectories)
                    .OrderByDescending(d => d.Length)
                    .ToList();
                ...
                foreach (var dir in dirs)
                {
                    try
                    {
                        // Pular o diretório que contÃ©m o desinstalador
        ...
        private void RemoveDirectoryAggressive(string directory)
        {
            ...
            try
            {
                ...
                // Tentar remover todos os arquivos com mÃºltiplas tentativas
                var files = Directory.GetFiles(directory, "*", SearchOption.AllDirectories);
                foreach (var file in files)
                {
                    for (int i = 0; i < 5; i++)
        ...
        /// <summary>
        /// método alternativo para forÃ§ar remoção de diretório com mais agressividade (otimizado)
        /// </summary>
