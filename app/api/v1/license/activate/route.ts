...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!licenseKey || !deviceId) {
            return NextResponse.json({ success: false, errorMessage: 'Chave e ID do dispositivo são obrigatórios' }, { status: 400 });
        }
        ...
        if (licError || !license) {
            return NextResponse.json({ success: false, errorMessage: 'Licença não encontrada', errorCode: 'LICENSE_NOT_FOUND' }, { status: 404 });
        }

        // 2. Verificar se está ativa e não expirada
        if (!license.is_active) {
            return NextResponse.json({ success: false, errorMessage: 'Licença inativa', errorCode: 'LICENSE_INACTIVE' });
        }

        if (license.expires_at) {
            const expiryDate = new Date(license.expires_at);
            if (expiryDate < new Date()) {
                return NextResponse.json({ success: false, errorMessage: 'Licença expirada', errorCode: 'LICENSE_EXPIRED' });
            }
        }
        ...
        if (license.user_id && user && license.user_id !== user.id) {
            return NextResponse.json({ success: false, errorMessage: 'Esta licença pertence a outra conta.', errorCode: 'FORBIDDEN' }, { status: 403 });
        }
        ...
        if (existingDevice) {
            ...
            return NextResponse.json({
                success: true,
                message: 'Dashboard: Dispositivo já autorizado.',
        ...
        if (license.license_type !== 'enterprise' && currentDevices >= maxDevices) {
            return NextResponse.json({
                success: false,
                errorMessage: `Limite de dispositivos atingido (${maxDevices}). Remova um dispositivo antigo no site.`,
        ...
        const { error: insertError } = await supabase
            ...
            .insert({
                ...
                device_id: deviceId,
                device_name: machineName || 'PC Desconhecido',
                machine_name: machineName,
                os_version: osVersion,
        ...
        if (insertError) {
            ...
            return NextResponse.json({ success: false, errorMessage: 'Erro ao registrar dispositivo no banco.' }, { status: 500 });
        }

        return NextResponse.json({
            success: true,
            message: 'Licença ativada com sucesso neste dispositivo!',
            licenseType: license.license_type,
            expiresAt: license.expires_at
        });
        ...
        return NextResponse.json({ success: false, errorMessage: 'Erro interno no servidor de ativação.' }, { status: 500 });
    }
}
