...
function SucessoContent() {
  ...
  const paymentType = searchParams?.get('payment_type') || null;
  
  // Logging para debug
  console.log('[SUCESSO PAGE] Parâmetros recebidos:', {
  ...
  useEffect(() => {
    console.log('[SUCESSO] useEffect disparado com:', { preferenceId, paymentId, collectionId });
    
    if (preferenceId || paymentId || collectionId) {
      console.log('[SUCESSO] Processando pagamento...');
      processPaymentReturn();
    } else {
      console.log('[SUCESSO] Nenhum parâmetro encontrado');
  ...
  useEffect(() => {
    if (!isPix && !license && retryAttempts < maxRetries && (preferenceId || paymentId || collectionId)) {
      const timer = setTimeout(() => {
        console.log(`[SUCESSO] Auto-retry ${retryAttempts + 1}/${maxRetries}...`);
        fetchLicense();
  ...
  }, [isPix, license, retryAttempts, preferenceId, paymentId, collectionId]);
  ...
  async function processPaymentReturn() {
    try {
      ...
      if (isSimulated) {
        console.log('[SUCESSO] Pagamento simulado detectado, pulando processamento do Mercado Pago...');
        // Para pagamentos simulados, ir diretamente para busca de licença
        await fetchLicense();
        return;
      }
  ...
  async function fetchLicense() {
    try {
      ...
      console.log('[SUCESSO] Buscando licença...', {
        preference_id: preferenceId,
        payment_id: paymentId,
        collection_id: collectionId,
      });
      ...
      if (!response.ok || !data.success) {
        ...
        if (data.message && data.message.includes('sendo gerada')) {
          ...
          if (data.retry_after) {
            setTimeout(() => {
              if (!license) { // Só tentar se ainda não tem licença
                fetchLicense();
              }
      ...
      if (data.license) {
        console.log('[SUCESSO] ✅ Licença encontrada:', data.license.license_key.substring(0, 20) + '...');
      ...
      console.error('[SUCESSO] Erro ao buscar licença:', err);
