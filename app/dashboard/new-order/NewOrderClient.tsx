  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-6">
        ...
        <p className="text-gray-400 font-black uppercase tracking-[0.3em] text-[10px] animate-pulse">Catalogando Serviços...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-4xl font-black text-gray-900 italic uppercase tracking-tighter">Novo <span className="text-[#FF4B6B] not-italic">Pedido</span></h2>
        </div>
        <p className="text-gray-500 font-bold text-xs uppercase tracking-widest pl-5 font-mono">Selecione módulos de infraestrutura para implantação</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 items-start">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {services.map((service, i) => (
              <motion.div
                ...
                className={`group relative p-8 rounded-[3rem] border transition-all duration-500 overflow-hidden ${transparencyMode ? 'voltris-glass' : 'bg-white border-gray-200 shadow-xl'} hover:border-[#FF4B6B]/40`}
              >
                ...
                <div className="relative z-10 space-y-6">
                   <div className="flex justify-between items-start">
                      ...
                      <span className="text-xl font-black text-gray-900 italic">R$ {service.price.toFixed(2)}</span>
                   </div>
                   
                   <div>
                     <h4 className="text-xl font-black text-gray-900 uppercase italic tracking-tighter leading-none mb-2">{service.name}</h4>
                     <p className="text-gray-500 text-xs font-bold leading-relaxed">{service.description}</p>
                   </div>
        ...
        <div className="xl:col-span-5 sticky top-28 flex flex-col gap-6">
          ...
          <div className={`p-8 rounded-[3.5rem] border ${transparencyMode ? 'voltris-glass' : 'bg-white border-gray-200 shadow-xl'}`}>
            <AnimatePresence mode="popLayout">
              {selectedServices.length === 0 ? (
                ...
                >
                  <div className="w-20 h-20 rounded-3xl bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-300">
                    <FiShoppingCart className="w-10 h-10" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-900 font-black uppercase italic tracking-tighter">Manifesto Vazio</p>
                    <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest">Aguardando seleção de módulos</p>
                  </div>
                </motion.div>
              ) : (
                <div className="space-y-6">
                  {selectedServices.map((item) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="p-5 rounded-3xl bg-gray-100 border border-gray-200 flex items-center gap-4 group"
                      >
                        <div className="w-12 h-12 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-400 group-hover:text-gray-900 transition-colors">
                           <FiPackage className="w-5 h-5" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <h5 className="text-xs font-black text-gray-900 uppercase italic tracking-tighter truncate">{service.name}</h5>
                          <p className="text-[10px] font-bold text-[#FF4B6B] uppercase tracking-widest">R$ {item.price.toFixed(2)}</p>
                        </div>
                        <div className="flex items-center gap-3">
                           <div className="flex items-center gap-2 p-1 bg-gray-200 rounded-xl border border-gray-300">
                              <button onClick={() => updateQuantity(item.service_id, item.quantity - 1)} className="w-7 h-7 rounded-lg hover:bg-gray-300 text-gray-900 transition-all">-</button>
                  ...
                  <div className="pt-8 border-t border-gray-200 space-y-8">
                    <div className="flex justify-between items-center">
                       <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Investimento Total</span>
                       <span className="text-3xl font-black text-gray-900 italic tracking-tighter">R$ {calculateTotal().toFixed(2)}</span>
                    </div>
