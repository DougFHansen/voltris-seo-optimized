...
export default function CompaniesPage() {
    ...
    const handleBuyLicenses = async () => {
        if (!company) return;
        try {
            const toastId = toast.loading("Processing Payment Uplink...");
            ...
            toast.error("Uplink Failure: " + err.message);
        }
    };

    const handleOptimizeAll = async () => {
        if (!company) return;
        if (!confirm("This will broadcast an OPTIMIZE sequence to ALL active nodes. Proceed?")) return;

        try {
            const toastId = toast.loading("Broadcasting Tactical Command...");
            ...
            if (!devices?.length) {
                toast.error("No active nodes found in this uplink.");
                return;
            }
            ...
            toast.dismiss(toastId);
            toast.success(`Mass Broadcast Sent: ${devices.length} Nodes Synchronized.`, {
                icon: '📡',
                style: { background: 'rgba(10, 10, 15, 0.9)', color: '#fff', border: '1px solid rgba(49, 168, 255, 0.2)' }
            });

        } catch (err: any) {
            toast.error("Broadcast Failure: " + err.message);
        }
    };
            ...
            <div className="flex flex-col items-center justify-center py-40 text-center gap-10">
                <div className={`p-10 rounded-[3rem] border border-white/5 flex flex-col items-center gap-8 ${transparencyMode ? 'voltris-glass' : 'bg-[#0A0A10]'}`}>
                    ...
                    <div className="space-y-4">
                        <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter leading-none">Fleet <span className="text-[#31A8FF]">Disconnected</span></h2>
                        <p className="text-white/20 font-bold text-xs uppercase tracking-[0.2em] max-w-md mx-auto">
                            Your account is not currently associated with a verified organization uplink. Establish a link to begin remote fleet management.
                        </p>
                    </div>
                    <button
        ...
        <div className="flex flex-col gap-10 relative w-full max-w-full overflow-hidden">
                ...
                <div className="space-y-2">
                   ...
                   <div className="flex flex-wrap items-center gap-4 pl-5">
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         <span className="text-[10px] font-black text-[#00FF88] uppercase tracking-[0.2em]">{stats.devices} / {company.max_devices} SLOTS ACTIVE</span>
                      </div>
                   </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                <StatCard
                    ...
                    icon={FiMonitor}
                    color="#31A8FF"
                    subtext={`${company.max_devices - stats.devices} Idle Slots`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className={`p-10 rounded-[3.5rem] border transition-all duration-500 overflow-hidden relative ${transparencyMode ? 'voltris-glass' : 'bg-[#12121A] border-white/5 shadow-2xl'}`}>
                    <div className="flex items-center gap-4 mb-10">
                       ...
                       <h3 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none">Remote <span className="text-[#8B31FF]">Directives</span></h3>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            <div className="flex items-center gap-4">
                               ...
                               <div className="flex flex-col">
                                  ...
                                  <span className="text-[9px] font-bold text-white/30 uppercase tracking-widest font-mono">Pulse to all nodes</span>
                               </div>
                            </div>
                        ...
                        <Link href="/dashboard/companies/devices" className="w-full text-left p-6 rounded-3xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] hover:border-[#FF4B6B]/40 transition-all flex items-center justify-between group">
                            <div className="flex items-center gap-4">
                               ...
                               <div className="flex flex-col">
                                  <span className="text-[11px] font-black text-white uppercase tracking-wider italic">Security Lockdown</span>
