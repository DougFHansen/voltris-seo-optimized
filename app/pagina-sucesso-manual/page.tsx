...
function PaginaSucessoManualCheck() {
  ...
  console.log('[PAGINA SUCESSO MANUAL] Parâmetros:', { preferenceId, paymentId, status });

  useEffect(() => {
    console.log('[PAGINA SUCESSO MANUAL] useEffect disparado');
  ...
  async function fetchLicense() {
    try {
      ...
      console.log('[PAGINA SUCESSO MANUAL] Buscando licença...');
      ...
      console.log('[PAGINA SUCESSO MANUAL] URL:', url);
      
      const response = await fetch(url);
      const data = await response.json();
      
      console.log('[PAGINA SUCESSO MANUAL] Resposta:', data);
      
      if (response.ok && data.success) {
        if (data.license) {
          console.log('[PAGINA SUCESSO MANUAL] Licença encontrada!');
          setLicense(data.license);
          setError(null);
        } else {
          console.log('[PAGINA SUCESSO MANUAL] Pagamento encontrado mas licença pendente');
          setError('Licença ainda sendo gerada. Aguarde alguns instantes e tente novamente.');
        }
      } else {
        console.log('[PAGINA SUCESSO MANUAL] Erro na resposta:', data.error || data.message);
        setError(data.error || data.message || 'Pagamento não encontrado');
      }
    } catch (err: any) {
      console.error('[PAGINA SUCESSO MANUAL] Erro:', err);
  ...
  async function checkPaymentStatus() {
    if (!preferenceId && !paymentId) {
      setError('Nenhum ID de pagamento encontrado');
      return;
    }
    ...
    try {
      ...
      console.log('[PAGINA SUCESSO MANUAL] Verificando status do pagamento...');
      
      // Chamar API para verificar status do pagamento no Mercado Pago
      const response = await fetch('/api/pagamento/check-status', {
        method: 'POST',
        headers: {
      ...
      console.log('[PAGINA SUCESSO MANUAL] Status check response:', data);
      
      if (response.ok && data.success) {
        // Se o pagamento foi aprovado, tentar buscar a licença novamente
        if (data.payment_approved) {
          console.log('[PAGINA SUCESSO MANUAL] Pagamento aprovado! Buscando licença...');
          await fetchLicense();
        } else {
          setError(`Pagamento ainda ${data.payment_status}. Aguarde a confirmação.`);
        }
      } else {
        setError(data.error || 'Erro ao verificar status do pagamento');
      }
    } catch (err: any) {
      console.error('[PAGINA SUCESSO MANUAL] Erro ao verificar status:', err);
      setError('Erro ao verificar status: ' + (err.message || 'Erro desconhecido'));
    } finally {
      setChecking(false);
    }
  }
