...
namespace VoltrisOptimizer.Services
{
    ...
    public partial class GamerOptimizerService
    {
        ...
        public async Task<bool> ActivateGamerModeAsync(string? gameExecutable = null, Action<int>? progressCallback = null)
        {
            return await Task.Run(async () =>
            {
                ...
                {
                    _logger.LogInfo("═══════════════════════════════════════════");
                    _logger.LogInfo("🎮 INICIANDO ATIVAÇÃO DO MODO GAMER");
                    _logger.LogInfo("═══════════════════════════════════════════");
                    ...
                    // ============================================
                    _logger.LogInfo("[Gamer] 📦 Executando otimizações pré-jogo...");
                    var preGameResult = await _sessionOptimizer.OnGameStartCleanupAndBoostAsync(gameExecutable);
                    
                    if (preGameResult.Success)
                    {
                        _logger.LogSuccess($"[Gamer] ✅ Pré-jogo concluído: {FormatBytes(preGameResult.TotalCleaned)} limpos, {preGameResult.ProcessesOptimized} processos otimizados");
                    ...
                    // ============================================
                    _logger.LogInfo("[Gamer] ⚡ Ativando otimizações de hardware...");
                    ...
                    if (!string.IsNullOrEmpty(gameExecutable))
                    {
                        ...
                        try
                        {
                            ...
                            if (proc != null)
                                {
                                    ...
                                    // Causa mais problemas (stuttering, latência) do que resolve
                            ...
                            {
                                _logger.LogWarning($"[Gamer] Erro ao aplicar otimizações extremas: {ex.Message}");
                            }
                        }
                    }
                    ...
                    { 
                        _logger.LogWarning($"[Gamer] Erro ao aplicar políticas unattend: {ex.Message}");
                    }
                    }
                ...
                {
                    _logger.LogError("Erro ao ativar modo gamer", ex);
                    progressCallback?.Invoke(100);
                    return false;
                }
            });
        ...
        public async Task<bool> DeactivateGamerModeAsync(Action<int>? progressCallback = null)
        {
            return await Task.Run(async () =>
            {
                ...
                {
                    _logger.LogInfo("═══════════════════════════════════════════");
                    _logger.LogInfo("🏁 INICIANDO DESATIVAÇÃO DO MODO GAMER");
                    _logger.LogInfo("═══════════════════════════════════════════");
                    ...
                    // ============================================
                    _logger.LogInfo("[Gamer] 🔄 Restaurando configurações do sistema...");
                    ...
                    if (_extremeModeEnabled)
                    {
                        ...
                        {
                            _logger.LogWarning($"[Gamer] Erro ao remover QoS: {ex.Message}");
                        }
                    }
                    progressCallback?.Invoke(70);
                    ...
                    // ============================================
                    _logger.LogInfo("[Gamer] 📦 Executando otimizações pós-jogo...");
                    var postGameResult = await _sessionOptimizer.OnGameExitCleanupAndRecoveryAsync(gameExe);
                    
                    if (postGameResult.Success)
                    {
                        _logger.LogSuccess($"[Gamer] ✅ Pós-jogo concluído: {postGameResult.ServicesRestored} serviços restaurados");
                    ...
                    _logger.LogSuccess("═══════════════════════════════════════════");
                    _logger.LogSuccess("🏁 MODO GAMER DESATIVADO COM SUCESSO!");
                    _logger.LogSuccess("═══════════════════════════════════════════");
                ...
                {
                    _logger.LogError("Erro ao desativar modo gamer", ex);
                    return false;
                }
            });
        }
                                ...
                                if (!string.IsNullOrEmpty(game.ExecutablePath))
                                {
                                    ...
                                    if (processes.Length > 0)
                                    {
                                        ...
                                        if (activated)
                                        {
                                            ...
                                            {
                                                NotificationManager.ShowSuccess(
                                                    "🎮 Modo Gamer Ativado Automaticamente",
    ...
    public class GameProfile
    {
        ...
        public string ExecutablePath { get; set; } = "";
        public GameProfileSettings Settings { get; set; } = new GameProfileSettings();
