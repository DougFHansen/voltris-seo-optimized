...
namespace VoltrisOptimizer.Services.Gamer
{
    ...
    public class LoadingPhaseOptimizerService : IDisposable
    {
            ...
            try
            {
                ...
                while (!ct.IsCancellationRequested && !_monitoredProcess.HasExited)
                {
                    try
                    {
                        ...
                        await Task.Delay(500, ct); // Verificar a cada 500ms
                    }
                    catch (Exception ex)
                    {
        ...
        /// <summary>
        /// Detecta início ou fim de fase de carregamento
        /// </summary>
        private void DetectLoadingPhase(LoadingMetrics metrics)
        {
            try
            {
                ...
                {
                    // Potencial início de carregamento
                    _lastLoadingStart = now;
                    _isLoadingPhase = true;
                    _logger.LogInfo("[LoadingOptimizer] Fase de carregamento detectada");
        ...
        private bool IsLoadingIndicatorsPresent(LoadingMetrics metrics)
        {
            ...
            // 1. Alta atividade de disco + baixa CPU (esperando I/O)
            // 2. Alto uso de memória (carregando assets)
            // 3. Padrões de espera prolongada
            ...
            var memorySpike = metrics.WorkingSetMb > 1000; // 1GB+ de memória em uso
            
            return highDiskActivity || lowCpuWithHighDisk || waitingPattern || memorySpike;
        }
