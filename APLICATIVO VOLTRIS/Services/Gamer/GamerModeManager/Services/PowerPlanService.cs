...
namespace VoltrisOptimizer.Services.Gamer.GamerModeManager
{
    ...
    public class PowerPlanService : IPowerPlanService
    {
        ...
        private bool TryCreateUltimatePerformancePlan(out string newGuid)
        {
            ...
            try
            {
                // Duplicar o plano High Performance
                var output = RunPowerCfg($"/duplicatescheme {StandardGuids[PowerPlanType.HighPerformance]}");
                ...
                if (match.Success)
                {
                    ...
                    _logger.LogInfo($"[PowerPlan] Plano Ultimate Performance criado: {newGuid}");
                    return true;
                }
            }
            catch (Exception ex)
