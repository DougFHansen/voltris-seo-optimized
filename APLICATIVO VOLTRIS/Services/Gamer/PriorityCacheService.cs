...
namespace VoltrisOptimizer.Services.Gamer
{
    ...
    public class PriorityCacheService : IDisposable
    {
                    ...
                    try
                    {
                        ...
                        if (process != null && !process.HasExited)
                        {
                            ...
                            if (process.PriorityClass != priority)
                            {
                                ...
                                _logger.LogInfo($"[PriorityCache] Prioridade de {process.ProcessName} alterada para {priority}");
                            }
                        }
                    }
                    catch
                    ...
                    try
                    {
                        ...
                        if (process != null && !process.HasExited)
                        {
                            ...
                            if (process.ProcessorAffinity != affinity)
                            {
                                ...
                                _logger.LogInfo($"[PriorityCache] Afinidade de processo {processId} atualizada");
                            }
                        }
                    }
                    catch
        ...
        private bool IsCriticalSystemProcess(string processName)
        {
            ...
            return criticalNames.Any(critical => processName.Contains(critical));
        }
