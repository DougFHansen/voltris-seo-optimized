...
namespace VoltrisOptimizer.Helpers
{
    public static class ShortcutHelper
    {
        ...
        public static void TryCreateShortcut()
        {
            try
            {
                VoltrisOptimizer.App.LoggingService?.LogInfo("[AUMID] Início criação de atalho");
                ...
                if (File.Exists(shortcutPath))
                {
                    try
                    {
                        ...
                        if (shellType == null)
                        {
                            VoltrisOptimizer.App.LoggingService?.LogWarning("[AUMID] WScript.Shell indisponível");
                            needCreate = true;
                        }
                        else
                        {
                            ...
                            if (shellObj == null)
                            {
                                VoltrisOptimizer.App.LoggingService?.LogWarning("[AUMID] Falha ao instanciar WScript.Shell");
                                needCreate = true;
                            }
                            else
                            {
                                ...
                                VoltrisOptimizer.App.LoggingService?.LogInfo($"[AUMID] Atalho existente Target={currentTarget}");
                ...
                if (needCreate)
                {
                    ...
                    if (shellType == null)
                    {
                        VoltrisOptimizer.App.LoggingService?.LogWarning("[AUMID] WScript.Shell indisponível para criar atalho");
                    }
                    else
                    {
                        var shellObj = Activator.CreateInstance(shellType);
                        if (shellObj == null)
                        {
                            VoltrisOptimizer.App.LoggingService?.LogWarning("[AUMID] Falha ao instanciar WScript.Shell para criar atalho");
                        }
                        else
                        {
                            dynamic shell = shellObj;
                            ...
                            shortcut.Save();
                            VoltrisOptimizer.App.LoggingService?.LogInfo($"[AUMID] Atalho criado em {shortcutPath}");

                            SetAppUserModelId(shortcutPath, AppUserModelId);
                            VoltrisOptimizer.App.LoggingService?.LogInfo($"[AUMID] Definido AppUserModelID='{AppUserModelId}'");
                        }
                    }
                }
                else
                {
                    try { SetAppUserModelId(shortcutPath, AppUserModelId); VoltrisOptimizer.App.LoggingService?.LogInfo($"[AUMID] Atualizado AppUserModelID em atalho existente"); } catch { }
                }
            }
            catch (Exception ex)
            {
                VoltrisOptimizer.App.LoggingService?.LogError($"[AUMID] Erro ao criar atalho: {ex.Message}", ex);
            }
        }
